...
class ComboBox(QComboBox):
    ...
    def __init__(self, *__args):
        ...
        if self.lineEdit():
            ...
            self.lineEdit().setStyleSheet(
                f"color: rgba({str(getForegroundColour(is_tuple=True)).replace('(', '').replace(')', '')}, {str(0.6)}); background: transparent; border: none;")
    ...
    def paintEvent(self, e):
        ...
        if self.lineEdit():
            ...
            self.lineEdit().setAttribute(Qt.WidgetAttribute.WA_MacShowFocusRect, False)
            self.lineEdit().setStyleSheet(
                f"color: rgba({str(getForegroundColour(is_tuple=True)).replace('(', '').replace(')', '')}, {str(self.property("Opacity") or 1.0)}); background: transparent; border: none;")
            self.lineEdit().setFont(self.font())
