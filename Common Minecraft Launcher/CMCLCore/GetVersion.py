...
def GetVersionsByMojangAPI(returns: str = "RETURN_DATA") -> Union[str, dict, list]:
    versions_url = f"https://launchermeta.mojang.com/mc/game/version_manifest.json"
    ...
    latest_version = None
    latest_version_data = {}
    if returns in ["RETURN_LATEST", "RETURN_LATEST_DATA"]:
...
def GetMinecraftClientDownloadUrl(version: Optional[Union[str]] = None) -> str:
    response = requests.get('https://launchermeta.mojang.com/mc/game/version_manifest.json')
    version_manifest = response.json()
    version_id = version or GetVersionsByMojangAPI(returns="RETURN_LATEST_DATA")
    version_info = None
...
def GetMinecraftServerDownloadUrl(version: Optional[Union[str]] = None) -> str:
    response = requests.get('https://launchermeta.mojang.com/mc/game/version_manifest.json')
    version_manifest = response.json()
    version_id = version or GetVersionsByMojangAPI(returns="RETURN_LATEST_DATA")
    version_info = None
    for version in version_manifest['versions']:
        if version['id'] == version_id:
            version_info = version
            break
    server_web_url = requests.get(version_info["url"])
