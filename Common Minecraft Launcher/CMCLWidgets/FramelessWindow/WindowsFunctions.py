...
def getMetricsDpi(hwnd, horizontal=True):
    if hasattr(windll.user32, "GetDpiForWindow"):
        ...
        windll.user32.GetDpiForWindow.argtype = [HWND]
        return windll.user32.GetDpiForWindow(HWND(hwnd))
...
def getWindowDpi(hwnd):
    ...
    windll.user32.GetDpiForWindow.restype = FLOAT
    windll.user32.GetDpiForWindow.argtypes = [HWND]
    return windll.user32.GetDpiForWindow(HWND(hwnd)) / 96.0
...
def getSystemAccentColour():
    ...
    colour = c_ulong()
    code = DwmGetColorizationColor(byref(colour), byref(c_bool()))
    
    if code:
        return QColor()
    
    return QColor(colour.value)
