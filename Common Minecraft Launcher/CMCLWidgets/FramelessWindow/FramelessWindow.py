...
class FramelessWindow(QWidget):
    def __init__(self, *__args):
        ...
        match self.__platform.system().lower():
            case "windows":
                ...
                self.__win32api = __import__("win32api")
                self.__dwmapi = self.__ctypes.WinDLL("dwmapi")
                self.setProperty("windowBorderAccentColour", getSystemAccentColour())
            case "darwin":
                self.__objc = __import__("objc")
                self.__cocoa = __import__("Cocoa")
                self.__nsWindow = self.__objc.objc_object(
                    c_void_p=self.winId().__int__()
                ).window()
            case "linux":
                pass
    ...
    def __updateShadow(self):
        match self.__platform.system().lower():
            case "windows":
                ...
                self.__dwmapi.DwmExtendFrameIntoClientArea(
                    int(self.winId()), self.__ctypes.byref(margins)
                )
            case "darwin":
    ...
    def __updateWin32Frameless(self):
        if self.__platform.system().lower() == "windows":
            super().setWindowFlag(
                Qt.WindowType.FramelessWindowHint, False
            )
            windowStyle = (
                    self.__win32gui.GetWindowLong(
                        int(self.winId()), self.__win32con.GWL_STYLE
                    )
            ...
            if self.windowFlags() & Qt.WindowType.WindowMinimizeButtonHint:
                windowStyle |= self.__win32con.WS_MINIMIZEBOX
            else:
                windowStyle &= ~self.__win32con.WS_MINIMIZEBOX
    ...
    def __updateNSWindowFrameless(self):
        if self.__platform.system().lower() == "darwin":
            self.__nsWindow.setStyleMask_(
                self.__nsWindow.styleMask()
                | self.__cocoa.NSWindowStyleMaskFullSizeContentView
            )
    ...
    def __updateNSWindowTitleBar(self):
        if self.__platform.system().lower() == "darwin":
            ...
            self.__nsWindow.standardWindowButton_(
                self.__cocoa.NSWindowCloseButton
            ).setHidden_(not self.systemTitleBarButtonVisible())
            self.__nsWindow.standardWindowButton_(
                self.__cocoa.NSWindowZoomButton
            ).setHidden_(not self.systemTitleBarButtonVisible())
            self.__nsWindow.standardWindowButton_(
                self.__cocoa.NSWindowMiniaturizeButton
            ).setHidden_(not self.systemTitleBarButtonVisible())
    ...
    def __updateNSWindowTitleBarRect(self):
        if self.__platform.system().lower() == "darwin":
            if self.systemTitleBarButtonVisible():
                close_button = self.__nsWindow.standardWindowButton_(
                    self.__cocoa.NSWindowCloseButton
                )
                miniaturise_button = self.__nsWindow.standardWindowButton_(
                    self.__cocoa.NSWindowMiniaturizeButton
                )
                zoom_button = self.__nsWindow.standardWindowButton_(
                    self.__cocoa.NSWindowZoomButton
                )
                ...
                titlebar_spacing = (
                        miniaturise_button.frame().origin.x - close_button.frame().origin.x
                )
                ...
                miniaturise_button_origin = self.__cocoa.NSPoint(
                    titlebar_centre.x() - miniaturise_button_width // 2,
                    titlebar_centre.y() - miniaturise_button_height // 2,
                )
                close_button_origin = self.__cocoa.NSPoint(
                    miniaturise_button_origin.x - titlebar_spacing,
                    miniaturise_button_origin.y,
                )
                zoom_button_origin = self.__cocoa.NSPoint(
                    miniaturise_button_origin.x + titlebar_spacing,
                    miniaturise_button_origin.y,
                )
    ...
    def event(self, a0):
        if self.isActiveWindow() or (
                a0.type() in [QEvent.Type.Paint, QEvent.Type.UpdateRequest, QEvent.Type.UpdateLater]):
            if hasattr(a0, "rect"):
                self.update(a0.rect())
            else:
                ...
                match winMsg.message:
                    ...
                    case self.__win32con.WM_NCHITTEST:
                        if self.resizeEnabled():
                            xPos, yPos = self.__win32gui.ScreenToClient(
                                winMsg.hWnd, self.__win32api.GetCursorPos()
                            )
                    ...
                    case self.__win32con.WM_NCCALCSIZE:
                        if winMsg.wParam:
                            rect = self.__ctypes.cast(
                                winMsg.lParam, self.__wintypes.LPNCCLACSIZE_PARAMS
                            ).contents.rgrc[0]
                        else:
                            rect = self.__ctypes.cast(
                                winMsg.lParam, self.__wintypes.LPRECT
                            ).contents
                        
                        if isMaximised(winMsg.hWnd) and not isFullScreen(winMsg.hWnd):
                            bx = getSystemMetrics(
                                int(winMsg.hWnd), win32con.SM_CYSIZEFRAME, True
                            ) + getSystemMetrics(int(winMsg.hWnd), 92, True)
                            by = getSystemMetrics(
                                int(winMsg.hWnd), win32con.SM_CXSIZEFRAME, False
                        ...
                        return True, win32con.WVR_REDRAW if winMsg.wParam else 0
    ...
    def setSystemTitleBarButtonVisible(self, value):
        if platform.system().lower() == "darwin":
            if value in [True, False, 1, 0]:
                ...
                raise TypeError(
                    f"'{type(value)}' object cannot be interpreted as a bool."
                )
        self.__updateWindowFrameless()
