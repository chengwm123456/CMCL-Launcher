    ...
    def paintEvent(self, a0):
        ...
        if self.title() or self.isCheckable():
            ...
            if self.property("frameOpacity"):
                ...
                rect2 = self.style().subControlRect(QStyle.ComplexControl.CC_GroupBox, op2,
                                                    QStyle.SubControl.SC_GroupBoxLabel).united(
                    self.style().subControlRect(QStyle.ComplexControl.CC_GroupBox, op2,
                                                QStyle.SubControl.SC_GroupBoxCheckBox))
                painter.save()
                painter.setOpacity(self.property("frameOpacity"))
                painter.setPen(getBorderColour())
                painter.drawLine(
                    QLine(QPoint(op2.rect.x() + 2, rect2.y() + max(rect2.height(), 16)),
                          QPoint(op2.rect.x() + op2.rect.width() - 2, rect2.y() + max(rect2.height(), 16))))
                painter.restore()
        if self.title():
            painter.save()
            painter.setOpacity(
